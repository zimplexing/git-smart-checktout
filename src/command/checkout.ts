    ...
    switch (item.label) {
        ...
        case 'Create new branch...':
            ...
            QuickItems.splice(4,0,{ 
                ...
                buttons: [{ iconPath: new vscode.ThemeIcon('notebook-edit')},{ iconPath: new vscode.ThemeIcon('notebook-delete-cell')}]
            });
            return;
        case 'Create new branch from...':
            ...
            const selectedBranchName = await vscode.window.showInputBox({ prompt: 'Enter new branch name', value: selectItem!.label, valueSelection: [0, selectItem!.label.length] });
...
const execSelectItemButton = async (selectedItem: vscode.QuickPickItemButtonEvent<vscode.QuickPickItem>, repo: Repository) => {
    const item = selectedItem.item;
    switch ((selectedItem.button.iconPath as vscode.ThemeIcon).id) {
        case 'notebook-edit':
            const branchName = await vscode.window.showInputBox({ prompt: 'Enter new branch name', value: item.label, valueSelection: [0, item.label.length] });
            if(!branchName) {
                return;
            }
            await repo.repository.renameBranch(branchName);
            QuickItems.splice(QuickItems.findIndex(i => i.label === item.label), 1, { label: branchName, description: item.description, detail: item.detail, iconPath: item.iconPath, buttons: item.buttons });
            return;
        case 'notebook-delete-cell':
            ...
            if (response === 'Yes') {
                try {
                    ...
                    QuickItems.splice(QuickItems.findIndex(i => i.label === item.label), 1);
                } catch (error) {
...
const initQuickItems = async () => {
    ...
    const localBranches = refs.filter(branch => branch.type === RefType.Head).slice(0,50);
    const remoteBranches = refs.filter(branch => branch.type === RefType.RemoteHead).slice(0,50);
    QuickItems.push(...await generateBranchItems(currentRepo, localBranches, new vscode.ThemeIcon('source-control-view-icon')));
    QuickItems.push({ kind: vscode.QuickPickItemKind.Separator, label: ''});
    QuickItems.push(...await generateBranchItems(currentRepo, remoteBranches, new vscode.ThemeIcon('extensions-install-local-in-remote')));
    statusBarItem.hide();
    syncTimer = setTimeout(initQuickItems, 1000 * 60 * 5);
};

const generateBranchItems = async (repo: Repository, branches: Branch[], icon: vscode.ThemeIcon): Promise<vscode.QuickPickItem[]> => {
    const items: vscode.QuickPickItem[] = [];
    for (const branch of branches) {
        const { message } = branch.commit ? await repo.getCommit(branch.commit) : {message: ''};
        items.push({ 
            label: branch.name ?? '', 
